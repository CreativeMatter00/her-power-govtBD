      ...
      <section className="border-t-2 border-brandLsPrimary mb-4 ">
        <div className="max-w-7xl container flex flex-col">
          ...
          <div className="mt-7 mb-2 flex flex-col ">
            <h1 className="text-3xl text-brandDs font-normal">{t("Success Stories")}</h1>
          ...
          {isLoading ? (
            ...
            <div>
              <div className="px-1 grid grid-cols-4 py-6 text-greyPrimary">
                ...
                <p>{t("Posted On")}</p>
                <p className="px-3 text-center">{t("Action")}</p>
              </div>
              {data?.data?.map((videoPost: any, index: number) => {
                return (
                  <div
                    key={videoPost?.story_pid}
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center px-3">
                      <Link
                        href={`/${locale}/success-stories/stories/stories-management/${videoPost?.story_pid}`}
                        className="text-link"
                      >
                        {t("View")}
                      </Link>
                      <Link
                        href={`/${locale}/success-stories/stories/stories-management/edit-stories/${videoPost?.story_pid}`}
                        className="text-link"
                      >
                        {t("Edit")}
                      </Link>
                      <button
                        ...
                        onClick={() => {
                          ...
                          setSelectedItem(videoPost?.story_pid);
                        }}
                      >
                        {t("Delete")}
                      </button>
        ...
        <Dialog open={editModalOpen} onOpenChange={setEditModalOpen}>
          ...
          <DialogContent className="bg-white w-[30vw]">
            ...
            <p className="mb-4 text-center text-gray-500 dark:text-gray-300">
              {t("Are you sure you want to delete this Stroy?")}
            </p>
