...
const FileInput: React.FC<IFiles> = ({
  ...
  const removeSelectedImage = () => {
    ...
    setImageSrc(null);
     if (setValue) {
    setValue(name.toLowerCase(), null); // clear from RHF
  }
  };
  ...
  return (
    <div>
      <label className="text-sm font-normal pb-2 pl-3 text-brandPrimary">
        {t("Challenges")} {t(`${name}`)} <span className="text-dangerPrimary">*</span>
      </label>
      <div
      ...
      >
        ...
        <div className="flex gap-2 items-center text-greyPrimary text-sm p-1">
          ...
          ; {t("Format")}:<span className="font-bold">{t("JPG,PNG")}</span>
        </div>
      </div>
