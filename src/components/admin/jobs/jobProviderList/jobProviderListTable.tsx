...
import { approveJobProvider, getJobProviders } from "@/api/api";
...
const JobProviderListTable = () => {
  // ============ DATA FETCHING ============
  const queryClient = useQueryClient();
  const { isLoading, data: allJobProviders } = useQuery({
    queryKey: ["allJobProviders"],
    queryFn: () => getJobProviders(),
  });
  const { mutateAsync, isPending, isError } = useMutation({
    mutationFn: ({ jobProvider_pid, data }: { jobProvider_pid: string | null; data: any }) =>
      approveJobProvider(jobProvider_pid, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["allJobProviders"] });
    },
  });
  console.log("alljobProviders", allJobProviders);

  const [jobProviderFlag, setJobProviderFlag] = useState<string | null>("");
  const [jobProviderId, setJobProviderId] = useState<string | null>("");
  ...
  const handleApproveStatus = (jobProviderInfo: any, flag: string) => {
    setJobProviderFlag(flag);
    setJobProviderId(jobProviderInfo?.jobprovider_pid)
  ...
  const data = useMemo(() => allJobProviders, [allJobProviders]);
  // ================ TABLE FUNCITONALITY ===============
  ...
  const handleApproveSubmit = (flag: string | null) => {
    ...
    mutateAsync({jobProvider_pid: jobProviderId, data: formData }).then((res) => {
        ...
        <div className="p-6">
          ...
          >
            ...
            <DialogContent className="sm:max-w-[525px] bg-white py-5">
              <DialogTitle className="">View Job Details</DialogTitle>
              <div className="grid grid-cols-1 gap-1">
                {viewTableData({ title: "Provider Name", value: viewData?.provider_name })}
          ...
          >
            <DialogContent className="sm:max-w-[425px] bg-white py-10">
              ...
              <DialogDescription className="text-center">
                Do you want to {jobProviderFlag === "c" ? "Cancel" : "Approve"} these
                job provider records?
              </DialogDescription>
              ...
              <DialogFooter>
                <Button
                  ...
                  type="submit"
                  className={`bg-${
                    jobProviderFlag === "c" ? "[#c9332e]" : "[#288d57]"
                  } text-white px-3 py-2`}
                  onClick={() => handleApproveSubmit(jobProviderFlag)}
                >
...
export default JobProviderListTable;
